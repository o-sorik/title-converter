...
test("returns disabled guidance when no output is available", () => {
  expect(getCopyFeedbackMessage("idle", false)).toBe("Type or paste text to enable copy.");
  expect(getCopyFeedbackMessage("success", false)).toBe("Type or paste text to enable copy.");
  expect(getCopyFeedbackMessage("error", false)).toBe("Type or paste text to enable copy.");
});
