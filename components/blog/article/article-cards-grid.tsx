  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
        {items.map((article) => (
          <article key={`${label}-${article.slug}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white transition-all duration-200 hover:-translate-y-1 hover:shadow-lg dark:border-zinc-700 dark:bg-zinc-900/60">
            <Link href={`/blog/${article.slug}`} className="block overflow-hidden">
              <Image src={article.image} alt={article.title} width={560} height={320} className="h-40 w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
            </Link>
            <div className="space-y-2 p-4">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-400">{label}</p>
